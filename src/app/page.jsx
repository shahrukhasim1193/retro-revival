...
const RIZQ_QUOTES={dashboard:{ar:'وَإِذَا سَأَلَكَ عِبَادِي عَنِّي فَإِنِّي قَرِيبٌ',en:'"And when My servants ask you about Me — indeed I am near." — Surah Al-Baqarah 2:186'},orders:{ar:'',en:'"Whoever is mindful of Allah, He will make a way out for them, and provide for them from sources they could never imagine." — Surah At-Talaq 65:2-3'},procurement:{ar:'وَفِي السَّمَاءِ رِزْقُكُمْ وَمَا تُوعَدُونَ',en:'"And in the heaven is your provision and whatever you are promised." — Surah Adh-Dhariyat 51:22'},expenses:{ar:'',en:'"Spend in charity, O son of Adam, and I shall spend on you." — Hadith Qudsi (Bukhari & Muslim)'},finance:{ar:'',en:'"Take account of yourselves before you are taken to account." — Umar ibn Al-Khattab (RA)'},settings:{ar:'',en:'"The strong believer is better and more beloved to Allah than the weak believer, while there is good in both." — Prophet Muhammad ﷺ (Muslim)'},calculator:{ar:'',en:'"If you are grateful, I will surely increase you in favour." — Surah Ibrahim 14:7'},inventory:{ar:'',en:'"Do not waste, for Allah does not love the wasteful." — Surah Al-A\'raf 7:31'}};
...
function exportCSV(filename,headers,rows){const esc=v=>{const s=String(v??'');return s.includes(',')||s.includes('"')||s.includes('\n')?`"${s.replace(/"/g,'""')}"`:s;};const csv=[headers.map(esc).join(','),...rows.map(r=>r.map(esc).join(','))].join('\n');const blob=new Blob([csv],{type:'text/csv'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=filename;a.click();URL.revokeObjectURL(url);}
...
export default function AppPage(){
  ...
  useEffect(()=>{const BANNER_ID='rr-jummah-apr10';const BANNER_EXPIRY='2026-04-11T10:00:00Z';const now=new Date();if(now>new Date(BANNER_EXPIRY))return;try{const dismissed=localStorage.getItem(BANNER_ID);if(!dismissed)setShowBanner(true);}catch{setShowBanner(true);}},[]);
...
function DashboardTab({categories,brands,procurements,dispatches,expenses,rate,sym,setActiveTab}){
  ...
  const periodGMVs=periods.map(p=>{const pD=dispatchedOrders.filter(d=>{const dt=new Date(d.dispatched_at);return dt>=p.start&&dt<=p.end;});const gmv=pD.reduce((s,d)=>s+(parseFloat(d.selling_price_gbp)||0),0);const ship=pD.reduce((s,d)=>s+(parseFloat(d.shipping_cost_gbp)||0),0);const rrG=gmv-ship;const comm=pD.reduce((s,d)=>{const rev=parseFloat(d.selling_price_gbp)||0;const sh=parseFloat(d.shipping_cost_gbp)||0;const pc=parseFloat(d.commission_pct)||0;return s+(rev-sh)*pc/100;},0);const cogs=pD.reduce((s,d)=>s+(d.dispatch_items||[]).reduce((ss,it)=>ss+it.quantity*parseFloat(it.unit_cost_gbp),0),0);const fE=(expenses||[]).filter(e=>{const dt=new Date(e.expense_date);return dt>=p.start&&dt<=p.end;});const directCosts=fE.filter(e=>e.category==='Direct Costs').reduce((s,e)=>s+parseFloat(e.amount_gbp),0);const opex=fE.filter(e=>e.category==='Opex').reduce((s,e)=>s+parseFloat(e.amount_gbp),0);const refunds=pD.reduce((s,d)=>s+(parseFloat(d.refund_amount_gbp)||0),0);const netE=rrG-comm;const grossProfit=netE-refunds-directCosts-cogs;const netProfit=grossProfit-opex;return{label:p.start.toLocaleDateString('en-GB',{month:'short'}),gmv,grossProfit,netProfit};});
...
function OrdersTab({supabase,user,categories,brands,salesChannels,procurements,dispatches,settings,loadAll,rate,sym}){
  ...
  function exportOrders(){const gn2=(list,id)=>list.find(i=>i.id===id)?.name||'';const rows=filtered.map(d=>{const its=(d.dispatch_items||[]).map(it=>`${gn2(categories,it.category_id)}/${gn2(brands,it.brand_id)} x${it.dispatched_qty||it.placed_qty||it.quantity}`).join('; ');const rev=parseFloat(d.selling_price_gbp)||0;const dg=parseFloat(d.dispatched_gmv_gbp)||rev;const ship=parseFloat(d.shipping_cost_gbp)||0;const rrG=rev-ship;const comm=parseFloat(d.commission_pct)||0;const commAmt2=rrG*comm/100;const netE=rrG-commAmt2;const cogs2=(d.dispatch_items||[]).reduce((s,it)=>s+(it.dispatched_qty||it.quantity)*parseFloat(it.unit_cost_gbp),0);const ref=parseFloat(d.refund_amount_gbp||0);const net=netE-ref-cogs2;return[new Date(d.placed_at||d.dispatched_at).toLocaleDateString('en-GB'),d.order_id,d.order_status,d.sales_channel_id?gn2(salesChannels,d.sales_channel_id):'',rev.toFixed(2),dg.toFixed(2),ship.toFixed(2),rrG.toFixed(2),comm,commAmt2.toFixed(2),netE.toFixed(2),cogs2.toFixed(2),ref.toFixed(2),net.toFixed(2),d.payment_status||'Pending',its,d.notes||'',d.logged_by_name||''];});exportCSV(`orders-${statusFilter.toLowerCase()}-${new Date().toISOString().slice(0,10)}.csv`,['Date','Order ID','Status','Channel','Order GMV','Dispatched GMV','Shipping','RR GMV','Comm %','Commission','Net Earnings','COGS','Refund','Net P&L','Payment','Items','Notes','Logged By'],rows);}
  ...
  return(<div>
    ...
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24,flexWrap:'wrap',gap:12}}>
      ...
      <div style={{display:'flex',gap:8,alignItems:'center'}}><input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder="Search orders..." style={{...inp,width:180,padding:'8px 12px',fontSize:13}}/><button onClick={exportOrders} style={{...btnS,padding:'8px 14px',fontSize:12}} title="Export CSV">Export</button><button onClick={()=>setShowNew(true)} style={{...btnP,padding:'10px 20px',display:'flex',alignItems:'center',gap:8}}><IconPlus/> New Order</button></div>
    </div>
    ...
    <Modal open={!!viewOrder} onClose={()=>setViewOrder(null)} title={viewOrder?`Order ${viewOrder.order_id}`:''} wide>
      {viewOrder&&(()=>{const rev=parseFloat(viewOrder.selling_price_gbp)||0;const dispGMV=parseFloat(viewOrder.dispatched_gmv_gbp)||rev;const ship=parseFloat(viewOrder.shipping_cost_gbp)||0;const rrGMV=rev-ship;const comm=parseFloat(viewOrder.commission_pct)||0;const commAmt2=rrGMV*comm/100;const netE=rrGMV-commAmt2;const its=viewOrder.dispatch_items||[];const orderCogs=its.reduce((s,it)=>s+(it.dispatched_qty||it.quantity)*parseFloat(it.unit_cost_gbp),0);const refund=parseFloat(viewOrder.refund_amount_gbp||0);const net=netE-refund-orderCogs;const unfulfilled=rev-dispGMV;
      return<div>
        ...
        <div style={{background:T.bg,borderRadius:10,padding:16,fontSize:14}}>
          ...
          <div style={{borderTop:`1px solid ${T.border}`,paddingTop:6,marginTop:4,display:'flex',justifyContent:'space-between',marginBottom:6}}><span style={{fontWeight:600,color:T.accent}}>Net Earnings</span><span style={{fontFamily:mono,fontWeight:600,color:T.accent}}>{sym}{fmt((netE*rate))}</span></div>
  ...
  return(<div>
    ...
    <div style={{...crd,padding:0,overflow:'hidden'}}>
      <div style={{display:'flex',gap:12,padding:'16px 20px',flexWrap:'wrap',alignItems:'center'}}>
        ...
        <div style={{flex:1}}/>
        <button onClick={()=>{const rows=filtered.map(s=>[gn(categories,s.category_id),gn(brands,s.brand_id),s.totalQty,s.reservedQty,s.totalQty+s.reservedQty,s.avgCost.toFixed(2)]);exportCSV(`inventory-${new Date().toISOString().slice(0,10)}.csv`,['Category','Brand','Available','Reserved','Total','AVCO Cost/Unit'],rows);}} style={{...btnS,padding:'8px 14px',fontSize:12}}>Export</button>
      </div>
...
function ProcurementTab({supabase,user,categories,brands,suppliers,procurements,loadAll,rate,sym}){
  ...
  return(<div>
    ...
    {sorted.length>0&&<div style={{...crd,padding:0,overflow:'hidden'}}><div style={{display:'flex',gap:12,padding:'16px 20px',flexWrap:'wrap',alignItems:'center'}}><h3 style={{fontFamily:dsp,fontSize:16,color:T.accent,margin:0}}>History</h3><select value={filterCat} onChange={e=>setFilterCat(e.target.value)} style={{...sel,width:'auto',minWidth:140,fontSize:12,padding:'6px 8px'}}><option value="">All Categories</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><select value={filterBrand} onChange={e=>setFilterBrand(e.target.value)} style={{...sel,width:'auto',minWidth:140,fontSize:12,padding:'6px 8px'}}><option value="">All Brands</option>{brands.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select>{(filterCat||filterBrand)&&<button onClick={()=>{setFilterCat('');setFilterBrand('');}} style={{background:'none',border:'none',color:T.accent,cursor:'pointer',fontSize:11,fontFamily:'inherit',textDecoration:'underline'}}>Clear</button>}<div style={{flex:1}}/><button onClick={()=>{const rows=filteredProcs.map(p=>[new Date(p.procured_at).toLocaleDateString('en-GB'),gn(categories,p.category_id),gn(brands,p.brand_id),p.supplier_id?gn(suppliers,p.supplier_id):'',parseFloat(p.unit_price_gbp).toFixed(2),p.quantity,p.remaining_qty,p.reserved_qty||0,p.is_paid?'Yes':'No',p.logged_by_name||'']);exportCSV(`procurement-${new Date().toISOString().slice(0,10)}.csv`,['Date','Category','Brand','Supplier','Unit Price GBP','Quantity','Remaining','Reserved','Paid','Logged By'],rows);}} style={{...btnS,padding:'6px 12px',fontSize:11}}>Export</button></div>
    <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}><thead><tr style={{borderBottom:`1px solid ${T.border}`}}>{['Date','Category','Brand','Supplier','Unit Price','Qty','Dispatched','Reserved','Remaining','Paid','By',''].map(h=><th key={h} style={_th}>{h}</th>)}</tr></thead><tbody>{filteredProcs.map(p=><tr key={p.id} style={{borderBottom:`1px solid ${T.borderLight}`}}><td style={{..._td,color:T.textSecondary}}>{new Date(p.procured_at).toLocaleDateString('en-GB')}</td><td style={_td}>{gn(categories,p.category_id)}</td><td style={_td}>{gn(brands,p.brand_id)}</td><td style={{..._td,fontSize:12,color:T.textMuted}}>{p.supplier_id?gn(suppliers,p.supplier_id):'—'}</td><td style={{..._td,fontFamily:mono}}>{sym}{fmt((parseFloat(p.unit_price_gbp)*rate))}</td><td style={_td}>{p.quantity}</td><td style={{..._td,fontWeight:600,color:T.textSecondary}}>{p.quantity - p.remaining_qty}</td><td style={{..._td,fontWeight:600,color:(p.reserved_qty||0)>0?T.amber:T.textFaint}}>{p.reserved_qty||0}</td><td style={{..._td,fontWeight:600,color:p.remaining_qty===0?T.textFaint:T.green}}>{p.remaining_qty}</td><td style={_td}>{p.is_paid?<span style={{color:T.green,fontSize:11}}>Yes</span>:<span style={{color:T.red,fontSize:11}}>No</span>}</td><td style={{..._td,fontSize:11,color:T.textMuted}}>{p.logged_by_name||'—'}</td><td style={_td}><div style={{display:'flex',gap:6}}><button onClick={()=>setEditProc({...p,unit_price_gbp:''+(parseFloat(p.unit_price_gbp)*rate).toFixed(2),_origQty:p.quantity,_origRemaining:p.remaining_qty})} style={{background:'none',border:'none',color:T.accent,cursor:'pointer',opacity:0.6,padding:4}}><IconEdit/></button><button onClick={()=>delProc(p.id)} style={{background:'none',border:'none',color:T.red,cursor:'pointer',opacity:0.5,padding:4}}><IconTrash/></button></div></td></tr>)}</tbody></table></div></div>}
...
function FinanceTab({supabase,dispatches,expenses,salesChannels,categories,brands,loadAll,rate,sym,currency}){
  ...
  const gmv=fDisp.reduce((s,d)=>s+(parseFloat(d.selling_price_gbp)||0),0);const totalShipping=fDisp.reduce((s,d)=>s+(parseFloat(d.shipping_cost_gbp)||0),0);const rrGMV=gmv-totalShipping;const totalCommission=fDisp.reduce((s,d)=>{const rev=parseFloat(d.selling_price_gbp)||0;const sh=parseFloat(d.shipping_cost_gbp)||0;const pct=parseFloat(d.commission_pct)||0;return s+(rev-sh)*pct/100;},0);
  ...
  const totalDispGMV=fDisp.reduce((s,d)=>s+(parseFloat(d.dispatched_gmv_gbp)||parseFloat(d.selling_price_gbp)||0),0);const totalUnfulfilled=gmv-totalDispGMV;
  ...
  return(<div>
    ...
    <div style={{display:'flex',gap:8,marginBottom:24,flexWrap:'wrap'}}><button onClick={()=>setView('pnl')} style={tabBtn('pnl')}>P&L Summary</button><button onClick={()=>setView('orders')} style={tabBtn('orders')}>Order P&L</button><button onClick={()=>setView('dispgmv')} style={tabBtn('dispgmv')}>Dispatched GMV</button><button onClick={()=>setView('payments')} style={tabBtn('payments')}>Payments</button></div>
    ...
    {view==='pnl'&&<div style={{...crd,padding:0,overflow:'hidden'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:14}}><thead><tr style={{borderBottom:`2px solid ${T.border}`}}><th style={{..._th,fontSize:13}}>Item</th><th style={{..._th,fontSize:13,textAlign:'right'}}>{currency}</th></tr></thead><tbody>
      ...
      <Row label="Net Earnings" val={nmv} bold highlight={T.accent}/><Row separator/>
    ...
    {view==='orders'&&<div style={{...crd,padding:0,overflow:'hidden'}}><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}><thead><tr style={{borderBottom:`2px solid ${T.border}`}}>{['Date','Order ID','Channel','Items','GMV','Ship','RR GMV','Comm','Refund','COGS','Net P&L','Status'].map(h=><th key={h} style={{..._th,fontSize:10}}>{h}</th>)}</tr></thead><tbody>{fDisp.length>0?fDisp.map(d=>{
      const rev=parseFloat(d.selling_price_gbp)||0;const ship=parseFloat(d.shipping_cost_gbp)||0;const rrG=rev-ship;const comm=parseFloat(d.commission_pct)||0;const commA=rrG*comm/100;const refund=parseFloat(d.refund_amount_gbp||0);const orderCogs=(d.dispatch_items||[]).reduce((s2,it)=>s2+(it.dispatched_qty||it.quantity)*parseFloat(it.unit_cost_gbp),0);const netE=rrG-commA;const net=netE-refund-orderCogs;const ps=d.payment_status||'Pending';const psCol=ps==='Paid'?T.green:ps==='Partial'?'#B8862D':T.textMuted;
    ...
    <Modal open={!!viewOrder} onClose={()=>setViewOrder(null)} title={viewOrder?`Order ${viewOrder.order_id}`:''}>
      {viewOrder&&(()=>{const rev=parseFloat(viewOrder.selling_price_gbp)||0;const ship=parseFloat(viewOrder.shipping_cost_gbp)||0;const rrG=rev-ship;const comm=parseFloat(viewOrder.commission_pct)||0;const cAmt=rrG*comm/100;const netE=rrG-cAmt;const its=viewOrder.dispatch_items||[];const oCogs=its.reduce((s2,it)=>s2+(it.dispatched_qty||it.quantity)*parseFloat(it.unit_cost_gbp),0);const ref=parseFloat(viewOrder.refund_amount_gbp||0);const net=netE-ref-oCogs;return<div>
        <div style={{background:T.bg,borderRadius:10,padding:16,fontSize:14}}>
    ...
    {view==='dispgmv'&&<div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))',gap:16,marginBottom:24}}>
        <div style={crd}><div style={{fontSize:11,color:T.textSecondary,textTransform:'uppercase',letterSpacing:1,marginBottom:6}}>Order GMV</div><div style={{fontSize:22,fontWeight:700,fontFamily:dsp,color:T.text}}>{sym}{fmt((gmv*rate),0)}</div></div>
        <div style={crd}><div style={{fontSize:11,color:T.textSecondary,textTransform:'uppercase',letterSpacing:1,marginBottom:6}}>Dispatched GMV</div><div style={{fontSize:22,fontWeight:700,fontFamily:dsp,color:T.green}}>{sym}{fmt((totalDispGMV*rate),0)}</div></div>
        <div style={{...crd,borderColor:totalUnfulfilled>0?'rgba(179,58,58,0.3)':T.border,background:totalUnfulfilled>0?'rgba(179,58,58,0.03)':T.bgCard}}><div style={{fontSize:11,color:totalUnfulfilled>0?T.red:T.textSecondary,textTransform:'uppercase',letterSpacing:1,marginBottom:6}}>Unfulfilled</div><div style={{fontSize:22,fontWeight:700,fontFamily:dsp,color:totalUnfulfilled>0?T.red:T.textMuted}}>{sym}{fmt((totalUnfulfilled*rate),0)}</div><div style={{fontSize:11,color:T.textMuted,marginTop:2}}>{gmv>0?(totalUnfulfilled/gmv*100).toFixed(1):0}% of Order GMV</div></div>
      </div>
      <div style={{...crd,padding:0,overflow:'hidden'}}><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}><thead><tr style={{borderBottom:`2px solid ${T.border}`}}>{['Date','Order ID','Channel','Order GMV','Dispatched GMV','Unfulfilled','% Fulfilled'].map(h=><th key={h} style={{..._th,fontSize:10}}>{h}</th>)}</tr></thead><tbody>
        {fDisp.length>0?fDisp.map(d=>{const rev=parseFloat(d.selling_price_gbp)||0;const dg=parseFloat(d.dispatched_gmv_gbp)||rev;const unf=rev-dg;const pct=rev>0?(dg/rev*100):100;const hasDiff=unf>0.5;return<tr key={d.id} style={{borderBottom:`1px solid ${T.borderLight}`,background:hasDiff?'rgba(179,58,58,0.04)':'transparent'}}>
          <td style={{..._td,color:T.textSecondary,fontSize:11}}>{new Date(d.dispatched_at).toLocaleDateString('en-GB')}</td>
          <td style={{..._td,fontFamily:mono,fontSize:11,color:T.accent,fontWeight:600}}>{d.order_id}</td>
          <td style={{..._td,fontSize:11}}>{d.sales_channel_id?gn(salesChannels,d.sales_channel_id):'—'}</td>
          <td style={{..._td,fontFamily:mono}}>{sym}{fmt((rev*rate))}</td>
          <td style={{..._td,fontFamily:mono,color:hasDiff?T.amber:T.text}}>{sym}{fmt((dg*rate))}</td>
          <td style={{..._td,fontFamily:mono,color:hasDiff?T.red:T.textMuted}}>{hasDiff?`${sym}${fmt((unf*rate))}`:'—'}</td>
          <td style={{..._td,fontFamily:mono,color:hasDiff?T.amber:T.green}}>{pct.toFixed(0)}%</td>
        </tr>;}):
        <tr><td colSpan={7} style={{..._td,textAlign:'center',color:T.textMuted,padding:30}}>No dispatched orders in this period</td></tr>}
      </tbody></table></div></div>
    </div>}
